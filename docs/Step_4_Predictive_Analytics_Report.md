...
**Baseline Comparison — Proving Model Value / 基线对比——证明模型价值**

A rigorous evaluation must compare the trained model against a naive baseline. The **majority-class baseline** (a dummy classifier that always predicts the dominant class) represents the minimum acceptable benchmark — a model with no intelligence would inherit this accuracy for free.

严谨的评估必须将训练模型与朴素基线进行比较。**多数类基线**（永远预测占多数类别的虚拟分类器）代表最低可接受标准——即零智能模型可免费获得的准确率。

| Metric (指标) | Majority-Class Baseline (多数类基线) | Our Logistic Regression (岭逻辑回归) | Lift (提升量) |
|---|---|---|---|
| Overall Accuracy (准确率) | 54.5% | **98.2%** | **+43.7 pts** |
| F1-Score (macro) | 0.354 (theoretical) | **0.980** | **+0.626** |
| ROC-AUC | 0.500 (random) | **0.995** | **+0.495** |

> **Conclusion:** The class split is 54.5% (Below Average) vs 45.5% (Above Average). A dummy model guessing "Below Average" every time would score 54.5% accuracy — no better than chance. Our model achieves 98.2% accuracy, a lift of **+43.7 percentage points**, with AUC=0.995 confirming that its discriminative power is not a result of class imbalance but genuine pattern recognition from the 19 input features.
> **结论：** 样本类别比例为 54.5%（低于均值）vs 45.5%（高于均值）。虚拟模型每次都猜"低于均值"可达到 54.5% 准确率——等同于随机猜测。我们的模型达到了 **98.2%** 准确率，提升幅度高达 **+43.7 个百分点**，AUC=0.995 证实了其判别能力来自 19 个输入特征的真实模式识别，而非类别失衡所致。
...
| Plot (图表) | Caption / 说明 |
|---|---|
| `Reg_ActualVsPredicted.png` | **Actual vs Predicted Exam Score.** The tight clustering on the lower part of the diagonal confirms the model accurately predicts the majority of students. However, it completely fails to capture the high-performing right tail: students scoring 85–100 are consistently under-predicted by the model, causing points to fall far below the $y=x$ ideal fit line. <br>**实际值与预测值对比散点图。** 在对角线下方的高度集聚证实了模型对绝大多数主体学生预测精准。然而，它**完全未能捕获表现优异的右尾群体**：考出 85-100 极高分的学生被模型严重低估，导致这一区间的散点大幅度跌落至 $y=x$ 理想拟合线下方。<br> |
| `Reg_ResidualDistribution.png` | **Residual Distribution Histogram.** While the mean is near zero, the distribution is **not normal**; it exhibits a massive leptokurtic peak near zero and a heavy right tail (reaching up to +30). This confirms mathematically what the scatter plot showed: the model has enormous negative error (under-prediction) for the small cohort of academic outliers. <br>**残差分布直方图。** 尽管均值接近零，但该分布**绝非正态分布**；它在 0 附近呈现出极端的狭峰形态，并伴随着直到 +30 的沉重右长尾。这从数学上印证了散点图的结论：模型对于那一小撮“学神级”离群点产生了巨大的低估误差（欠拟合）。 |
...
| `Clf_ROCCurve.png` | **ROC Curve (AUC = 0.995).** The curve rises sharply to the top-left corner with negligible area remaining **above** the curve, approaching a perfect classifier. An AUC of 0.995 means the model virtually always ranks an at-risk student higher in risk than a non-at-risk student. **ROC曲线（AUC = 0.995）。** 曲线急剧攀升至左上角，曲线**上方**剩余面积极小（即曲线与完美点(0,1)之间的空白极小），趋近于完美分类器表现。AUC=0.995 意味着模型几乎总是能将高风险学生的预测风险评分排在低风险学生之上。 |
| `Clf_PRCurve.png` | **Precision-Recall Curve (AUC = 0.996).** Maintains near-perfect precision across all recall levels, confirming that when the model flags a student as "at-risk", this flag is almost always correct — critical for resource-constrained intervention systems. **精确率-召回率曲线（AUC = 0.996）。** 在所有召回率水平下始终保持接近完美的精确率，证实每当模型将一名学生标记为"高风险"时，该标记几乎总是准确的——这对于资源有限的干预体系至关重要。 |
