...
3. **Time constraints.** Senior administrators rarely have time to review a full analytical report. The single-page dashboard format delivers the core narrative — from performance baseline through behavioural drivers to equity diagnostics — in a format that can be consumed in minutes rather than hours.
...
**Sidebar Filters.** Three global filters — Institution Type (Public/Private), Family Income (High/Medium/Low), and Motivation Level (High/Medium/Low) — allow the user to slice the entire dashboard simultaneously. When a filter is applied, all 13 charts and all 5 summary metrics update in real time, enabling rapid hypothesis testing. For example, an administrator can select "Low" income to immediately see how the score distribution, behavioural trends, and equity patterns change for the most disadvantaged cohort.

**KPI Summary Strip.** Five headline metrics are displayed prominently at the top: Students Analysed, Mean Exam Score, Average Attendance, Average Study Hours per Week, and Internet Access Rate. These update dynamically with the filters, providing instant context before the user explores the detailed charts below.
