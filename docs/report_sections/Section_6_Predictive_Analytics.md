...
Given a student's profile across all 19 features, the model predicts their expected exam score. This enables academic planners to estimate cohort-level grade distributions before exams take place and identify students whose predicted scores fall below acceptable thresholds. It also allows simulation of hypothetical interventions (e.g., "If this student's attendance increases from 70% to 85%, how much would their predicted score improve?").
...
**Table 7: Regression Model Candidates**
...
**Table 8: Classification Model Candidates**
...
**Figure 24:** Actual vs. Predicted Exam Score. The dashed red line represents a perfect prediction; the shaded band shows ±1 point tolerance.
...
**Figure 25:** Distribution of prediction errors (Actual − Predicted).
...
**Figure 26:** Feature importance ranking showing which factors most strongly drive the predicted score.
...
- **Previous Scores**, **Access to Resources**, and **Parental Involvement** form a second tier (ranked 3rd--5th), each contributing meaningfully but well below the top two.
- **Tutoring Sessions** ranks 6th, followed by a group of lower-tier features including Peer Influence, Motivation Level, and Family Income.
- The remaining features (Sleep Hours, Physical Activity, Gender, School Type) have the smallest coefficients and do not appear in the top 15 shown in the chart.

### Classification Evaluation

[Insert Figure: Clf_ConfusionMatrix.png]

**Figure 27:** Confusion matrix showing the proportion of correct and incorrect predictions for each class.

Both classes are predicted with high accuracy (Below Average: 97.4%, Above Average: 98.8%). The low error rates confirm that the model is reliable at identifying both above-average and below-average students.

[Insert Figure: Clf_ROCCurve.png]

**Figure 28:** ROC curve (AUC = 0.995). The dashed diagonal represents a model making random guesses.
...
**Figure 29:** Precision-Recall curve showing the trade-off between catching at-risk students and avoiding false alarms.
...
**Figure 30:** Feature importance ranking for the classification model.
